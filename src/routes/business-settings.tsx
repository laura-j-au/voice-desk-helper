...
function BusinessSettings() {
 ...
 return <><PageHeading title="Business settings" description="Manage the approved information the AI can use when answering calls." />
   ...
   <div className="grid gap-6 xl:grid-cols-2"><Card className="rounded-lg shadow-sm"><CardHeader><CardTitle className="text-base">Business details</CardTitle></CardHeader><CardContent className="space-y-5"><Field label="Business name"><Input aria-label="Business name" value={form.business} onChange={(e)=>setField("business",e.target.value)}/></Field><Field label="AI greeting"><Textarea aria-label="AI greeting" rows={4} value={form.greeting} onChange={(e)=>setField("greeting",e.target.value)}/></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Notification email"><Input aria-label="Notification email" type="email" value={form.email} onChange={(e)=>setField("email",e.target.value)}/></Field><Field label="Business phone"><Input aria-label="Business phone" value={form.phone} onChange={(e)=>setField("phone",e.target.value)}/></Field></div><Field label="Opening hours">{form.hours.map((h,i)=><Input aria-label={`Opening hours ${i+1}`} key={i} value={h} onChange={(e)=>setField("hours",form.hours.map((v,n)=>n===i?e.target.value:v))} className="mb-2"/>)}</Field></CardContent></Card>
