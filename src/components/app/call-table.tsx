...
export function CallTable({ rows, compact = false }: { rows: CallRecord[]; compact?: boolean }) {
  ...
  return (
    <Table>
      ...
      <TableBody>{rows.map((call) => <TableRow key={call.id} className="group cursor-pointer" tabIndex={0} onClick={() => navigate({ to: "/calls/$callId", params: { callId: call.id } })} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") navigate({ to: "/calls/$callId", params: { callId: call.id } }); }}>
        ...
        <TableCell className="pr-5 text-right"><Button variant="ghost" size="icon" asChild title={`View ${call.caller}'s call`} onClick={(event) => event.stopPropagation()}><Link to="/calls/$callId" params={{ callId: call.id }} aria-label={`View call from ${call.caller}`}><ArrowUpRight /></Link></Button></TableCell>
      </TableRow>)}</TableBody>
    </Table>
  );
}
